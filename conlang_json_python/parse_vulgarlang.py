...
def main(argv):
    ...
    spelling_pronunciation_rules = parse_spelling_rules(spelling_rule_list)
    
    # Parse the grammar rules.  This will build the affix_map and part of the lexicon
    affix_map,lexicon_fragment1 = parse_grammar_rules(grammar,part_of_speech_set,spelling_pronunciation_rules,
        {
    ...
    lexicon_fragment2 = parse_word_list(word_list,affix_map,spelling_pronunciation_rules)
    ...
    if arguments.decline:
        add_lexicon = []
        for word in lexicon:
            add_lexicon += decline_word(word,affix_map,spelling_pronunciation_rules)
        lexicon += add_lexicon
    ...
    language_structure = {
                            ...
                            'native_name_english':spell_word(vulgarlang['ipaLangName']['value'].strip(), spelling_pronunciation_rules).capitalize(),
                         }
    if arguments.voice:
    ...
    language_structure['spelling_pronunciation_rules'] = spelling_pronunciation_rules
...
def parse_spelling_rules(spelling_rule_list):
    ...
    spelling_pronunciation_rules = []
    spelling_pronunciation_rules.append({'phoneme':'ˈ','romanization':'','spelling_regex':'ˈ','pronunciation_regex':'&&&&&&'})
    spelling_pronunciation_rules.append({'phoneme':'ˌ','romanization':'','spelling_regex':'ˌ','pronunciation_regex':'&&&&&&'})
    ...
    for spelling_rule in spelling_rule_list:
        ...
        if sound_map_list_addition:
            spelling_pronunciation_rules += sound_map_list_addition
    ...
    for sound_map in spelling_pronunciation_rules:
    ...
    return spelling_pronunciation_rules
...
# Vulgarlang spelling rules
    ...
    if not re.match(r'^\s*/.*',spelling_rule):
        ...
        if group_repl_match1:
            ...
            spelling_pronunciation_rules.append(sound_map)
...
def parse_word_list(word_list,affix_map,spelling_pronunciation_rules):
    lexicon_fragment = []
    for word in word_list:
        ...
        for english in english_list:
            lexicon_fragment.append(LEXICON_ENTRY(phonetic.strip(),spell_word(phonetic,spelling_pronunciation_rules),english.strip(),part_of_speech.strip(),'root'))
    return lexicon_fragment
                 
#end def parse_word_list(word_list,affix_map,spelling_pronunciation_rules)
...
def parse_derivational_affix_list(derivational_affix_list,spelling_pronunciation_rules,patterns):
    ...
    for derivational_affix in derivational_affix_list:
        ...
        if vowel_prefix_match:
            t_spelling_add = spell_word(vowel_prefix_match.group(1),spelling_pronunciation_rules)
            f_spelling_add = spell_word(vowel_prefix_match.group(2),spelling_pronunciation_rules)
            map_entry = {
                'type':'PREFIX',
            ...
            t_spelling_add = spell_word(vowel_suffix_match.group(1),spelling_pronunciation_rules)
            f_spelling_add = spell_word(vowel_suffix_match.group(2),spelling_pronunciation_rules)
            map_entry = {
                'type':'SUFFIX',
                'pronunciation_regex':IPA_VOWELS_PATTERN+'$',
                'spelling_regex':SPELLING_VOWEL_PATTERN+'$',
            ...
            t_spelling_add = spell_word(consonant_prefix_match.group(1),spelling_pronunciation_rules)
            f_spelling_add = spell_word(consonant_prefix_match.group(2),spelling_pronunciation_rules)
            map_entry = {
                'type':'PREFIX',
            ...
            t_spelling_add = spell_word(consonant_suffix_match.group(1),spelling_pronunciation_rules)
            f_spelling_add = spell_word(consonant_suffix_match.group(2),spelling_pronunciation_rules)
            map_entry = {
                'type':'SUFFIX',
                'pronunciation_regex':IPA_CONSONANT_PATTERN+'$',
                'spelling_regex':SPELLING_CONSONANT_PATTERN+'$',
            ...
            map_entry = {
                ...
                'spelling_add':spell_word(prefix_match.group(1),spelling_pronunciation_rules),
            }
        elif suffix_match:
            map_entry = {
                'type':'SUFFIX',
                'pronunciation_add':suffix_match.group(1),
                'spelling_add':spell_word(suffix_match.group(1),spelling_pronunciation_rules),
            }
        else:
...
def parse_grammar_rules(grammar,part_of_speech_set,spelling_pronunciation_rules,patterns):
    IPA_VOWELS_PATTERN        = patterns['IPA_VOWELS_PATTERN']
    IPA_CONSONANT_PATTERN     = patterns['IPA_CONSONANT_PATTERN']
    SPELLING_VOWEL_PATTERN     = patterns['SPELLING_VOWEL_PATTERN']
    SPELLING_CONSONANT_PATTERN = patterns['SPELLING_CONSONANT_PATTERN']
        ...
        if 'insert' in entry:
            ...
            if line.startswith('TABLE TYPE'):
                ...
                if (table_type == 'affix') or (table_type == 'prefix') or (table_type == 'postfix'):
                    ...
                    affix_type, map_entry = parse_affix_rule(affix_rule,spelling_pronunciation_rules,patterns)
                    if affix_type == '':
                    ...
                    spelled = spell_word(phonetic,spelling_pronunciation_rules)
                    if part_of_speech == '':
...
def parse_affix_rule(affix_rule,spelling_pronunciation_rules,patterns):
    IPA_VOWELS_PATTERN        = patterns['IPA_VOWELS_PATTERN']
    IPA_CONSONANT_PATTERN     = patterns['IPA_CONSONANT_PATTERN']
    SPELLING_VOWEL_PATTERN     = patterns['SPELLING_VOWEL_PATTERN']
    SPELLING_CONSONANT_PATTERN = patterns['SPELLING_CONSONANT_PATTERN']
    ...
    if vowel_prefix_match:
        ...
        t_spelling_add = spell_word(vowel_particle_match.group(1).strip(),spelling_pronunciation_rules)
        f_spelling_add = spell_word(vowel_particle_match.group(2).strip(),spelling_pronunciation_rules)
        affix_type = 'particle'
        map_entry = {
            'pronunciation_regex':'^'+IPA_VOWELS_PATTERN,
            'spelling_regex':'^'+SPELLING_VOWEL_PATTERN,
        ...
        t_spelling_add = spell_word(vowel_suffix_match.group(1).strip(),spelling_pronunciation_rules)
        f_spelling_add = spell_word(vowel_suffix_match.group(2).strip(),spelling_pronunciation_rules)
        affix_type = 'suffix'
        map_entry = {
            'pronunciation_regex':IPA_VOWELS_PATTERN+'$',
            'spelling_regex':SPELLING_VOWEL_PATTERN+'$',
        ...
        t_spelling_add = spell_word(consonant_prefix_match.group(1).strip(),spelling_pronunciation_rules)
        f_spelling_add = spell_word(consonant_prefix_match.group(2).strip(),spelling_pronunciation_rules)
        affix_type = 'prefix'
        map_entry = {
            'pronunciation_regex':'^'+IPA_CONSONANT_PATTERN,
            'spelling_regex':'^'+SPELLING_CONSONANT_PATTERN,
        ...
        t_spelling_add = spell_word(consonant_particle_match.group(1).strip(),spelling_pronunciation_rules)
        f_spelling_add = spell_word(consonant_particle_match.group(2).strip(),spelling_pronunciation_rules)
        affix_type = 'particle'
        map_entry = {
            'pronunciation_regex':'^'+IPA_CONSONANT_PATTERN,
            'spelling_regex':'^'+SPELLING_CONSONANT_PATTERN,
        ...
        t_spelling_add = spell_word(consonant_suffix_match.group(1).strip(),spelling_pronunciation_rules)
        f_spelling_add = spell_word(consonant_suffix_match.group(2).strip(),spelling_pronunciation_rules)
        affix_type = 'suffix'
        map_entry = {
            'pronunciation_regex':IPA_CONSONANT_PATTERN+'$',
            'spelling_regex':SPELLING_CONSONANT_PATTERN+'$',
        ...
        map_entry = {
            'pronunciation_add':prefix_match.group(1),
            'spelling_add':spell_word(prefix_match.group(1).strip(),spelling_pronunciation_rules),
        }
    elif particle_match:
        affix_type = 'particle'
        map_entry = {
            'pronunciation_add':particle_match.group(1),
            'spelling_add':spell_word(particle_match.group(1).strip(),spelling_pronunciation_rules),
        }
    elif suffix_match:
        affix_type = 'suffix'
        map_entry = {
            'pronunciation_add':suffix_match.group(1),
            'spelling_add':spell_word(suffix_match.group(1).strip(),spelling_pronunciation_rules),
        }
