...
# Decline a word.
def decline_word(word,affix_map,spelling_pronunciation_rules,derived_word=False):
    ...
    if isinstance(word,str):
        ...
        word_source_metatdata = LEXICON_ENTRY(phonetic=phonetic,spelled=spell_word(phonetic, spelling_pronunciation_rules),english=english_list[0],part_of_speech=part_of_speech,declension=[]).as_map()
...
def derive_words(derived_word_list,derivational_affix_map,lexicon,affix_map,spelling_pronunciation_rules,decline=True):
    ...
    for words in derived_word_list:
        ...
        for eng in english.split(','):
            eng = eng.strip()
            entry = LEXICON_ENTRY(phonetic,spell_word(phonetic,spelling_pronunciation_rules),eng, part_of_speech, ['root'],derived_word=True,declined_word=False,metadata={'source':{'derrived_word':words}})
            ...
            if decline:
                word_lexicon_fragment = decline_word(new_word_line,affix_map,spelling_pronunciation_rules,derived_word=True)
...
def spell_word(phonetic, spelling_pronunciation_rules):
    spelled = phonetic
